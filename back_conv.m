...
function [dzdx, dzdw, dzdb] = forw_conv(xmat, wfilt, bias, ymat, dzdy)
...
%testy = reshape( M * wfilt(:) + bias , size(ymat) )
...
%testy = reshape( M * xmat(:) + bias , size(ymat) )


%compute derivatives with respect to scalar bias b (this is easy)
dzdb = 1*sum(sum(dzdy));
